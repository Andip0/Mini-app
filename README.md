...
How it works
You submit numbers through the frontend
FastAPI receives the request, validates it with Pydantic, and creates a Celery chord
The chord fans out one task per number — all running in parallel across Celery workers
Redis acts as the message broker (holds tasks in a queue) and result backend (stores finished results)
The frontend polls GET /job-status/{job_id} every 800ms until results are ready
Results and summary stats are displayed on the page
Tech stack
